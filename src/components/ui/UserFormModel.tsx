...
const UserFormModal: React.FC<UserFormModalProps> = ({
  ...
  const handleAssign = () => {
    if (selectedUsers.length > 0 && selectedTask) {
      onAssignUsersToTask(
        selectedUsers.map((user) => user.id.toString()),
        selectedTask
      );
      setIsOpen(false);
    }
    ...
    <dialog ref={modalRef} className="modal dark:bg-white/10">
      <div className="modal-box p-5 dark:bg-gray-900">
        <h3 className="font-bold text-lg mb-4 dark:text-white">
          {" "}
          {t("dashboard.userFormModal.title")}
        </h3>

        {/* User Search and Selection */}
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 dark:text-white mb-2">
            {t("dashboard.userFormModal.searchUsers")}
          </label>
          <input
            type="text"
            placeholder={t("dashboard.userFormModal.searchPlaceholder")}
            value={userSearch}
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 dark:text-white mb-2">
            {t("dashboard.userFormModal.selectTask")}
          </label>
          <select
          ...
          >
            <option value="">
              {t("dashboard.userFormModal.selectTaskPlaceholder")}
            </option>
            {tasks.map((task) => (
        ...
        <div className="flex justify-end space-x-4">
          <button className="btn" onClick={() => setIsOpen(false)}>
            {t("dashboard.userFormModal.closeButton")}
          </button>
          <button
          ...
          >
            {t("dashboard.userFormModal.assignButton")}
          </button>
        </div>
      </div>
    </dialog>
